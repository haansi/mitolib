...
```bash
java -jar greenVC-0.1.5.jar haplocheck --in variants.txt --out haplogrepinput.hsd   --VAF 0.05 
...
```bash
java -jar greenVC-0.1.5.jar haplocheck-mtDNA-Server --in heteroplasmies.txt --out haplogrepinput.hsd  --VAF 0.05 
...
```bash
java -jar greenVC-0.1.5.jar lofreq --in inputfile.vcf --out haplogrepinput.hsd 
```
